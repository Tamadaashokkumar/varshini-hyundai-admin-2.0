...
export default function OrdersPage() {
    ...
    if (socket && isConnected) {
      ...
      socket.on("new_order", (data: any) => {
        ...
        audio.play().catch((err) => {
          ...
          toast.error("🔊 Click explicitly on page to enable sound!");
        });
  ...
  const handleDownloadInvoice = async (order: Order) => {
    try {
      ...
      // API call
      const response = await downloadInvoice(order._id, order.orderNumber);

      // ✅ FIX: TypeScript error నివారించడానికి 'as any' వాడుతున్నాం.
      // API నుండి వచ్చే రెస్పాన్స్ Axios object అయితే .data ఉంటుంది, లేకపోతే direct blob ఉండొచ్చు.
